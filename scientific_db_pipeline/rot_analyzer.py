...
def analyze_rot_distribution():
    ...
    try:
        ...
        stats = session.query(
            ...
            func.median(Paper.rot_score).label('median_rot'),
            func.stddev(Paper.rot_score).label('stddev_rot'),
...
def group_papers_by_rot(high_rot_threshold=None, low_rot_threshold=None):
    ...
    try:
        ...
        if high_rot_threshold is None or low_rot_threshold is None:
            median_rot = session.query(func.median(Paper.rot_score)).filter(
...
def export_rot_groups_to_csv():
    ...
    try:
        ...
        # Group papers
        high_rot_df = df[df['rot_score'] >= median_rot].copy()
        low_rot_df = df[df['rot_score'] < median_rot].copy()
        
        # Add group labels
        high_rot_df['rot_group'] = 'High ROT'
        low_rot_df['rot_group'] = 'Low ROT'
        
        # Export to CSV
        high_rot_df.to_csv('high_rot_papers.csv', index=False)
        low_rot_df.to_csv('low_rot_papers.csv', index=False)
        
        # Combine all papers with group labels
        all_papers_df = pd.concat([high_rot_df, low_rot_df])
        all_papers_df.to_csv('all_papers_with_rot_groups.csv', index=False)
        
        logging.info(f"Exported {len(high_rot_df)} High ROT papers to 'high_rot_papers.csv'")
        logging.info(f"Exported {len(low_rot_df)} Low ROT papers to 'low_rot_papers.csv'")
        logging.info(f"Exported all {len(all_papers_df)} papers to 'all_papers_with_rot_groups.csv'")
...
def main():
    ...
    """
    print("=== SCIENTIFIC PAPERS ROT ANALYSIS ===")
